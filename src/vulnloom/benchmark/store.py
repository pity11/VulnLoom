"""Crash-safe benchmark checkpoints and immutable local result artifacts."""
...
from .models import BenchmarkArtifact, BenchmarkOutcome, BenchmarkPlan, BenchmarkResult


class BenchmarkIdempotencyConflict(ValueError):
    """An idempotency key was reused for a different benchmark plan."""


class BenchmarkRecoveryRequired(RuntimeError):
    """An unfinished benchmark checkpoint requires explicit operator recovery."""
...
class BenchmarkStore:
    ...
    def claim(self, plan: BenchmarkPlan, *, now: datetime) -> BenchmarkClaim:
        try:
            ...
            return BenchmarkClaim(created=True)
            ...
            if row["plan_id"] != plan.plan_id:
                raise BenchmarkIdempotencyConflict(
                    "benchmark idempotency key was reused for different content"
                ) from None
            if row["state"] == "started":
                raise BenchmarkRecoveryRequired(
                    "benchmark has an unfinished STARTED checkpoint; replay is refused"
                ) from None
    ...
    def complete(self, outcome: BenchmarkOutcome, *, completed_at: datetime) -> None:
        with self.connection:
        ...
        if changed != 1:
            raise BenchmarkRecoveryRequired("benchmark STARTED checkpoint is unavailable")
...
class BenchmarkArtifactStore:
    def __init__(self, root: Path, *, max_artifact_bytes: int = 2 * 1024 * 1024):
        if max_artifact_bytes <= 0:
            raise ValueError("Benchmark artifact size limit must be positive")
        self.root = root.resolve()
        self.objects = self.root / "objects"
        self.objects.mkdir(parents=True, exist_ok=True, mode=0o700)
        self.max_artifact_bytes = max_artifact_bytes

    def put(self, result: BenchmarkResult) -> BenchmarkArtifact:
        ...
        temporary = Path(tempfile.mkdtemp(prefix="benchmark-", dir=self.objects))
        try:
    ...
    def read_result(self, artifact: BenchmarkArtifact) -> BenchmarkResult:
        self._verify(artifact)
        return BenchmarkResult.model_validate_json(self._read(self.root / artifact.json_ref))

    def read_markdown(self, artifact: BenchmarkArtifact) -> str:
        self._verify(artifact)
        return self._read(self.root / artifact.markdown_ref).decode()

    def _verify(self, artifact: BenchmarkArtifact) -> None:
        directory = self.objects / artifact.result_digest
        try:
            metadata = os.lstat(directory)
        except OSError as exc:
            raise ValueError("Benchmark object directory is unavailable or unsafe") from exc
        if not stat.S_ISDIR(metadata.st_mode) or {item.name for item in directory.iterdir()} != {
            "result.json",
            "result.md",
        }:
            raise ValueError("Benchmark object directory is unavailable or unsafe")
        ...
        if hashlib.sha256(json_bytes).hexdigest() != artifact.json_sha256:
            raise ValueError("Benchmark JSON integrity check failed")
        if hashlib.sha256(markdown_bytes).hexdigest() != artifact.markdown_sha256:
            raise ValueError("Benchmark Markdown integrity check failed")
        result = BenchmarkResult.model_validate_json(json_bytes)
        if canonical_digest(result.model_dump(mode="python")) != artifact.result_digest:
            raise ValueError("Benchmark result content identity mismatch")

    def _read(self, path: Path) -> bytes:
        if not hasattr(os, "O_NOFOLLOW"):
            raise ValueError("platform cannot enforce no-follow Benchmark reads")
        try:
            descriptor = os.open(path, os.O_RDONLY | os.O_NOFOLLOW)
        except OSError as exc:
            raise ValueError("Benchmark artifact is unavailable or unsafe") from exc
        try:
            metadata = os.fstat(descriptor)
            if not stat.S_ISREG(metadata.st_mode) or metadata.st_size > self.max_artifact_bytes:
                raise ValueError("Benchmark artifact is unavailable or unsafe")
    ...
    def _check_size(self, content: bytes) -> None:
        if len(content) > self.max_artifact_bytes:
            raise ValueError("Benchmark artifact exceeds the configured size limit")
...
def _render_markdown(result: BenchmarkResult) -> str:
    ...
    lines = [
        "# VulnLoom Offline Benchmark Result",
        "",
        f"- Gate: `{result.gate_status.value}`",
        f"- Suite: `{result.suite_id}`",
        f"- Plan: `{result.plan_id}`",
        f"- Candidate recall: `{metrics.candidate_recall:.9f}`",
        f"- Finding precision: `{metrics.finding_precision:.9f}`",
        f"- Duplicate rate: `{metrics.duplicate_rate:.9f}`",
        f"- Evidence completeness: `{metrics.evidence_completeness:.9f}`",
        f"- Policy violations: `{metrics.policy_violation_count}`",
        f"- Total elapsed ms: `{metrics.total_elapsed_ms}`",
        f"- Cost per Finding (microunits): `{metrics.cost_per_finding_microunits}`",
        "",
        "## Gate violations",
        "",
    ]
