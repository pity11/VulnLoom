"""Crash-safe report checkpoints and immutable local draft artifacts."""

from __future__ import annotations

import hashlib
import os
import shutil
import sqlite3
import stat
import tempfile
from dataclasses import dataclass
from datetime import datetime
from pathlib import Path

from vulnloom.domain.digests import canonical_digest
from vulnloom.domain.models import Report
from vulnloom.evidence.redaction import Redactor

from .models import ReportArtifact, ReportDraftPlan, ReportOutcome
from .render import render_markdown


class ReportIdempotencyConflict(ValueError):
    """A report idempotency key was reused for a different sealed plan."""


class ReportRecoveryRequired(RuntimeError):
    """An unfinished report checkpoint requires explicit operator recovery."""


@dataclass(frozen=True)
class ReportClaim:
    created: bool
    outcome: ReportOutcome | None = None


class ReportDraftStore:
    def __init__(self, path: Path):
        self.path = path
        self.path.parent.mkdir(parents=True, exist_ok=True)
        self.connection = sqlite3.connect(path)
        self.connection.row_factory = sqlite3.Row
        self._create_schema()

    def _create_schema(self) -> None:
        self.connection.execute(
            """
            CREATE TABLE IF NOT EXISTS report_executions (
                plan_id TEXT PRIMARY KEY,
                idempotency_key TEXT NOT NULL UNIQUE,
                state TEXT NOT NULL CHECK(state IN ('started', 'completed')),
                started_at TEXT NOT NULL,
                completed_at TEXT,
                outcome_json TEXT
            )
            """
        )
        self.connection.commit()

    def claim(self, plan: ReportDraftPlan, *, now: datetime) -> ReportClaim:
        try:
            with self.connection:
                self.connection.execute(
                    """
                    INSERT INTO report_executions (
                        plan_id, idempotency_key, state, started_at
                    ) VALUES (?, ?, 'started', ?)
                    """,
                    (plan.plan_id, plan.idempotency_key, now.isoformat()),
                )
            return ReportClaim(created=True)
        except sqlite3.IntegrityError:
            row = self.connection.execute(
                """
                SELECT * FROM report_executions
                WHERE plan_id = ? OR idempotency_key = ?
                """,
                (plan.plan_id, plan.idempotency_key),
            ).fetchone()
            if row is None:
                raise
            if row["plan_id"] != plan.plan_id:
                raise ReportIdempotencyConflict(
                    "report idempotency key was reused for different content"
                ) from None
            if row["state"] == "started":
                raise ReportRecoveryRequired(
                    "report has an unfinished STARTED checkpoint; automatic replay is refused"
                ) from None
            return ReportClaim(
                created=False,
                outcome=ReportOutcome.model_validate_json(row["outcome_json"]),
            )

    def complete(self, outcome: ReportOutcome) -> None:
        with self.connection:
            changed = self.connection.execute(
                """
                UPDATE report_executions
                SET state = 'completed', completed_at = ?, outcome_json = ?
                WHERE plan_id = ? AND state = 'started'
                """,
                (outcome.completed_at.isoformat(), outcome.model_dump_json(), outcome.plan_id),
            ).rowcount
        if changed != 1:
            raise ReportRecoveryRequired("report STARTED checkpoint is unavailable")

    def has_checkpoint(self, plan_id: str) -> bool:
        return (
            self.connection.execute(
                "SELECT 1 FROM report_executions WHERE plan_id=?", (plan_id,)
            ).fetchone()
            is not None
        )

    def load_completed(self, plan_id: str) -> ReportOutcome:
        row = self.connection.execute(
            "SELECT * FROM report_executions WHERE plan_id=?", (plan_id,)
        ).fetchone()
        if row is None:
            raise ValueError("report outcome is unavailable")
        if row["state"] != "completed" or row["outcome_json"] is None:
            raise ReportRecoveryRequired("report has an unfinished STARTED checkpoint")
        outcome = ReportOutcome.model_validate_json(row["outcome_json"])
        if outcome.plan_id != plan_id or outcome.completed_at.isoformat() != row["completed_at"]:
            raise ReportRecoveryRequired("report completed checkpoint drifted")
        return outcome

    def close(self) -> None:
        self.connection.close()

    def __enter__(self) -> ReportDraftStore:
        return self

    def __exit__(self, *_: object) -> None:
        self.close()


class ReportArtifactStore:
    def __init__(self, root: Path, *, max_artifact_bytes: int = 2 * 1024 * 1024):
        if max_artifact_bytes <= 0:
            raise ValueError("Report artifact size limit must be positive")
        self.root = root.resolve()
        self.objects = self.root / "objects"
        self.objects.mkdir(parents=True, exist_ok=True, mode=0o700)
        self.max_artifact_bytes = max_artifact_bytes

    def put(self, report: Report) -> ReportArtifact:
        narrative = (
            report.title,
            report.summary,
            report.impact,
            report.remediation,
            *report.reproduction,
            *(section.text for section in report.sections),
        )
        redactor = Redactor()
        if any(redactor.text(value) != value for value in narrative):
            raise ValueError("Report contains content that has not passed redaction")
        report_digest = canonical_digest(report.model_dump(mode="python"))
        destination = self.objects / report_digest
        json_bytes = (report.model_dump_json(indent=2) + "\n").encode()
        markdown_bytes = render_markdown(report).encode()
        self._check_size(json_bytes)
        self._check_size(markdown_bytes)
        artifact = ReportArtifact(
            report_digest=report_digest,
            markdown_sha256=hashlib.sha256(markdown_bytes).hexdigest(),
            json_sha256=hashlib.sha256(json_bytes).hexdigest(),
            markdown_ref=f"objects/{report_digest}/report.md",
            json_ref=f"objects/{report_digest}/report.json",
        )
        if destination.exists():
            self._verify(artifact)
            return artifact

        temporary = Path(tempfile.mkdtemp(prefix="report-", dir=self.objects))
        try:
            self._write(temporary / "report.json", json_bytes)
            self._write(temporary / "report.md", markdown_bytes)
            os.chmod(temporary, 0o500)
            try:
                os.rename(temporary, destination)
            except FileExistsError:
                self._verify(artifact)
            self._verify(artifact)
            return artifact
        finally:
            if temporary.exists():
                os.chmod(temporary, 0o700)
                shutil.rmtree(temporary)

    def read_report(self, artifact: ReportArtifact) -> Report:
        self._verify(artifact)
        return Report.model_validate_json(self._read(self.root / artifact.json_ref))

    def read_markdown(self, artifact: ReportArtifact) -> str:
        self._verify(artifact)
        return self._read(self.root / artifact.markdown_ref).decode()

    def _verify(self, artifact: ReportArtifact) -> None:
        directory = self.objects / artifact.report_digest
        try:
            directory_metadata = os.lstat(directory)
        except OSError as exc:
            raise ValueError("Report object directory is unavailable or unsafe") from exc
        if not stat.S_ISDIR(directory_metadata.st_mode) or {
            item.name for item in directory.iterdir()
        } != {"report.json", "report.md"}:
            raise ValueError("Report object directory is unavailable or unsafe")
        json_bytes = self._read(self.root / artifact.json_ref)
        markdown_bytes = self._read(self.root / artifact.markdown_ref)
        if hashlib.sha256(json_bytes).hexdigest() != artifact.json_sha256:
            raise ValueError("Report JSON integrity check failed")
        if hashlib.sha256(markdown_bytes).hexdigest() != artifact.markdown_sha256:
            raise ValueError("Report Markdown integrity check failed")
        report = Report.model_validate_json(json_bytes)
        if canonical_digest(report.model_dump(mode="python")) != artifact.report_digest:
            raise ValueError("Report content identity mismatch")

    def _read(self, path: Path) -> bytes:
        if not hasattr(os, "O_NOFOLLOW"):
            raise ValueError("platform cannot enforce no-follow Report reads")
        try:
            descriptor = os.open(path, os.O_RDONLY | os.O_NOFOLLOW)
        except OSError as exc:
            raise ValueError("Report artifact is unavailable or unsafe") from exc
        try:
            metadata = os.fstat(descriptor)
            if not stat.S_ISREG(metadata.st_mode) or metadata.st_size > self.max_artifact_bytes:
                raise ValueError("Report artifact is unavailable or unsafe")
            with os.fdopen(descriptor, "rb", closefd=False) as handle:
                content = handle.read(self.max_artifact_bytes + 1)
        finally:
            os.close(descriptor)
        self._check_size(content)
        return content

    def _check_size(self, content: bytes) -> None:
        if len(content) > self.max_artifact_bytes:
            raise ValueError("Report artifact exceeds the configured size limit")

    @staticmethod
    def _write(path: Path, content: bytes) -> None:
        descriptor = os.open(path, os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o400)
        with os.fdopen(descriptor, "wb") as handle:
            handle.write(content)
            handle.flush()
            os.fsync(handle.fileno())
