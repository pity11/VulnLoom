...
- R8 新增 `EndpointReconReservation` 显式状态机。`active` 预算只能逐次前进到 `consumed`，操作员取消或 deadline
  过期只释放未消费部分；已经写入 Flow action ledger 的动作不会回退。多个 active reservation 在同一 Flow 内
  事务性竞争剩余 RoE 预算，跨 checkpoint 的未消费 reservation 也不会被重复分配。
...
- R9 新增 `EndpointCheckSchedule`、checkpoint 与 run 状态机。Schedule 固定绑定 Scope/version、规范 Target、
  operator、精确路径、UTC 固定周期、有效窗口和每轮预算；触发只物化新的 Flow、Seed Set 与 Endpoint Recon Plan，
  不直接调用 Broker、adapter 或网络。
- 每个 due slot 只有一个内容寻址 run；完成重试只读重放，STARTED 必须显式恢复且最多三次。上一轮 Flow、
  reservation 或 Recon run 未达到可证明的 clean terminal 状态时拒绝下一轮，避免并发重复 Flow 与重复副作用。
...
S1.2 第二个纵切已达到 `local_tested`：六类内容寻址的资源压力资格协议固定 PID、FD、输出、临时盘、内存和
超时进程组的精确终态，并要求 boundary observed、完整 cleanup 和容器缺失；本地有界 Docker canary 已分别
触发 cgroup PID、nofile、tmpfs 和 OOM 边界。rootless opt-in 组合测试已接入 Phase 3，但尚未取得远端 run
证据，因此不得标记为生产通过。下一纵切补运行中取消接口与取消后的进程组/残留对象回收。
